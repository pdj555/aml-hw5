#!/bin/bash
#
# Run PNN with Sinh-Arcsinh distribution for rainfall prediction with 8 rotations
#
#SBATCH --partition=normal
#SBATCH --nodes=1
#SBATCH --cpus-per-task=20
#SBATCH --mem=32G
#SBATCH --output=mesonet_rotations_%j_stdout.txt
#SBATCH --error=mesonet_rotations_%j_stderr.txt
#SBATCH --time=8:00:00
#SBATCH --job-name=mesonet_rotations
...
# Create a combined analysis script to generate the required figures
echo "Creating combined analysis script"
cat > ${RESULTS_DIR}/analyze_rotations.py << 'EOF'
"""
...
python ${RESULTS_DIR}/analyze_rotations.py --results_dir "${RESULTS_DIR}" --output_dir "${RESULTS_DIR}/combined_analysis"

echo "All rotations completed. Results saved to ${RESULTS_DIR}"
